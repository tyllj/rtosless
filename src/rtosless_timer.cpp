...
static inline uint64_t rtosless_now(uint8_t res) {
    return (res == TIMER_RESOLUTION_MILLIS) ? millis() : micros();
}
...
extern "C" {
    ...
    void timer_continue(uint8_t index) {
        ...
        if (timer_get_trigger_state(t->flags) == TIMER_STATE_PAUSED && t->interval > 0) {
            uint64_t now = rtosless_now(timer_get_resolution(t->flags));
            if (now >= t->exec_time) {
                uint64_t missed = (now - t->exec_time) / t->interval;
                           ...
                           uint64_t phase, uint8_t resolution, uint8_t priority, uint8_t one_shot)
    {
        ...
        uint32_t ps = rtosless_enter_critical();
        rtosless_timer_t* t = &timers[index];
        if (!t->handler && !t->member_stub) { rtosless_exit_critical(ps); return; }
    ...
    void timer_process_queue(void) {
        uint64_t now_ms = rtosless_now(TIMER_RESOLUTION_MILLIS);
        uint64_t now_us = rtosless_now(TIMER_RESOLUTION_MICROS);
        ...
        for (uint8_t i = 0; i < TIMER_MAX_COUNT; ++i) {
            ...
            if ((t->handler || t->member_stub) && timer_get_trigger_state(t->flags) == TIMER_STATE_RUNNING) {
                uint64_t now = (timer_get_resolution(t->flags) == TIMER_RESOLUTION_MILLIS) ? now_ms : now_us;
                if (now >= t->exec_time) {
                    should_invoke = 1;
